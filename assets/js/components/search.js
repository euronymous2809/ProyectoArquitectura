...
const pokemonItem = (pokemon) => {
  ...
  const img = $(`<a href="#" data-toggle="modal" data-ruta="${imgRuta}" data-id="${pokemon.entry_number}" data-name="${pokemon.pokemon_species.name}" data-target="#pokeDetail"><img src="https://assets.pokemon.com/assets/cms2/img/pokedex/detail/${imgRuta}.png" alt=""></a>`);
  const divDetail = $('<div class="poke-info"></div>');
  const name = $(`<h5 class="poke-name gris-oscuro">${pokemon.pokemon_species.name}</h5>`);
  const divEnlaces = $('<div class="ter-block"></div');
